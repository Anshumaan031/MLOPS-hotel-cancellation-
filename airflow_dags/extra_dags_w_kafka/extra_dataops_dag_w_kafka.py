...
def produce_message_task():
    producer = KafkaProducer(bootstrap_servers=KAFKA_BROKER_URL)
    message = 'Last Task of DataOps DAG is finished'
    producer.send(MLOPS_TOPIC, message.encode('utf-8'))
    producer.flush()
    time.sleep(5)
    producer.close()
...
with DAG(
    ...
    produce_message_task = PythonOperator(
        ...
        python_callable=produce_message_task
    )
    ...
    handling_totalspecialrequests_col_nulls >> create_bookingdate_col >> create_season_col >> handling_target_feature >> data_profiling_after >> produce_message_task
